...
void experiment() {
    ...
    for (int n = 1; n <= MAX_N; ++n) {
        ...
        double timePerm2 = std::chrono::duration<double, std::milli>(end - start).count();
    ...
    gp << "plot 'result/data.txt' using 1:2 with linespoints title 'getAllPerms', \\\n";
    gp << "     'result/data.txt' using 1:3 with linespoints title 'getPerm2'\n";
    gp.close();

    system("gnuplot result/plot.gp");
}
